...
const schema = z.object({
  ...
  description: z.string().min(4, "Descreva o servico."),
  durationInMinutes: z.coerce.number().min(5, "A duracao minima e de 5 minutos."),
  price: z.coerce.number().min(0, "O preco deve ser maior ou igual a zero."),
});
...
export default function ServiceFormModal({
  ...
  useEffect(() => {
    ...
    reset({
      ...
      durationInMinutes: Number(initialData?.durationInMinutes ?? initialData?.duration ?? 30),
    ...
    >
      <div className="flex min-h-full items-start justify-center pt-2 sm:items-center sm:pt-4">
        ...
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-5">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-slate-900">
              {isEditing ? "Atualizar servico" : "Cadastrar servico"}
            </h2>
          </div>
          <button
            type="button"
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="grid min-h-0 gap-5 overflow-y-auto overflow-x-hidden px-6 py-6">
          <div className="grid min-w-0 gap-4 md:grid-cols-[minmax(0,1.4fr)_minmax(0,0.8fr)_minmax(0,0.8fr)]">
            ...
            <label className="grid min-w-0 gap-2 text-sm font-semibold text-slate-700">
              Duracao
              <input type="number" min="5" {...register("durationInMinutes")} className={inputClass} />
              {errors.durationInMinutes ? <span className="text-sm text-rose-500">{errors.durationInMinutes.message}</span> : null}
            </label>

            <label className="grid min-w-0 gap-2 text-sm font-semibold text-slate-700">
              Preco
              <input type="number" min="0" step="0.01" {...register("price")} className={inputClass} />
              {errors.price ? <span className="text-sm text-rose-500">{errors.price.message}</span> : null}
            </label>
          </div>
          ...
          <div className="flex flex-col gap-3 border-t border-slate-200 pt-5 sm:flex-row sm:justify-end">
            ...
            >
              ...
              {submitting ? "Salvando..." : isEditing ? "Salvar alteracoes" : "Cadastrar servico"}
            </button>
          </div>
        </form>
        </div>
